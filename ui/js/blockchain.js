...
function show_details(event, id){								//build the block details html
	console.log("Inside show_details, event :"+event);
	console.log("Inside show_details, id :"+id);
	var left = event.pageX - $('#details').parent().offset().left - 50;
	if(left < 0) left = 0;
	var ccid = formatCCID(blocks[id].blockstats.transactions[0].type, blocks[id].blockstats.transactions[0].txid, atob(blocks[id].blockstats.transactions[0].chaincodeID));
...
function new_block(newblck){									//rec a new block
	console.log("inside blockchain.js")
